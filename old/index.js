...
function config(cfg){
  console.log('RIOTHING starting');
  Object.assign(CFG, cfg);
  //CFG.pub   = path.resolve(CFG.pub);
  console.log(CFG);
  console.log(getActions(CFG.pub + CFG.actionDir));
...
function getActions(dir){
  return readDir(dir).then(console.log)
  //return 'no actions';
}
...
function renderHTML(opts, tagName = 'html'){
  ...
  let client = [];
  CFG.internalClient && client.push(fs.readFileSync(__dirname + CFG.client, 'utf8'));
  client.push(`new Riothing({ stores: ${JSON.stringify(stores)}, state: '/content.json' });`);
  opts.CLIENT = toBase64(client.join(';'));
  return  `
    <!DOCTYPE html>
